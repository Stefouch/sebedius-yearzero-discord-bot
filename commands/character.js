...
async function characterSwitch(ctx, name, lang = 'en') {
	...
	if (!name) {
		...
		return await ctx.reply(
			`${__('ccharacter-active-character', lang)}: **${activeCharacter.name}**`,
			{ deleteAfter: 20 },
		);
	}

	const selectedCharacter = await getSelection(ctx, characters.map(c => [c.name, c]), { lang });
...
async function characterDelete(ctx, lang = 'en') {
	...
	const selectedCharacter = await getSelection(ctx, characters.map(c => [`${c.name} (${c.id})`, c]), { lang });

	const confirmation = await confirm(
		ctx,
		`⚠️ ${__('ccharacter-delete-confirmation', lang).replace('{character_name}', selectedCharacter.name)}`,
		true,
	);
	...
	if (confirmation) {
		...
		if (deleted) {
			return await ctx.send(
				`🧼 ${__('ccharacter-delete-confirmation', lang).replace('{character_name}', selectedCharacter.name)}`,
			);
		}
	}
