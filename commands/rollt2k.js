...
module.exports = {
	...
	async execute(args, ctx) {
		...
		const t2kRollRegex = /^(?:d?\d{1,2}|[abcdf]|\d{1,2}[dg]|g\d{1,2})+$/i;
		...
		// If not, checks if the first argument is a YZ roll phrase.
			...
			for (const arg of rollargv._) {
				...
				if (t2kRollRegex.test(arg)) {
					...
					const diceCouples = arg.match(/(d?\d{1,2})|([abcdf])|(\d{1,2}[dg])|(g\d{1,2})/gi);
